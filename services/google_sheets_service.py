...
- get_all_active_processes — все активные процессы (для группового режима)
"""
...
class GoogleSheetsService:
    ...
    def _migrate_english_statuses(self) -> None:
        """Миграция: заменяет английские статусы на русские (после переключения языка)."""
        try:
            all_rows = self._sheet.get_all_values()
            status_col = COLUMNS.index("Статус") + 1  # 1-based
            migrated = 0
    ...
    def _get_or_create_sheet(self) -> gspread.Worksheet:
        ...
        try:
            ...
            # Проверяем заголовки — обновляем если не совпадают
            try:
    ...
    def _col_index(self, col_name: str) -> int:
        """Возвращает 1-based индекс колонки по имени."""
    ...
    def add_time_log(
        ...
        with self._lock:
            ...
            logger.info(
                "Запись создана: user=%s process=%s id=%s",
                user_name, process, row_id,
            )
            return row
    ...
    def finish_time_log(
        ...
        with self._lock:
            ...
            start_time = datetime.strptime(
                row_data[COLUMNS.index("Начало")], DATETIME_FORMAT
            )
            ...
            logger.info(
                "Процесс завершён: user_id=%s duration=%s мин",
                telegram_id, duration_minutes,
            )
    ...
    def get_today_logs(self, telegram_id: int) -> list[dict]:
        """Возвращает все записи пользователя за сегодняшний день."""
        ...
        for row in all_rows[1:]:
            if len(row) < len(COLUMNS):
                continue
            if (
                row[COLUMNS.index("ID сотрудника")] == str(telegram_id)
                and row[COLUMNS.index("Дата")] == today
            ):
                results.append(
                    {
    ...
    def get_all_active_processes(self) -> list[dict]:
        """Возвращает все текущие активные процессы всех сотрудников."""
        all_rows = self._sheet.get_all_values()
        results: list[dict] = []
    ...
    def get_today_all_logs(self) -> list[dict]:
        """Возвращает все записи всех сотрудников за сегодняшний день."""
        today = self._now().strftime(DATE_FORMAT)
        all_rows = self._sheet.get_all_values()
    ...
    def _find_active_row(
        ...
        for i in range(len(all_rows) - 1, 0, -1):
            ...
            if (
                row[COLUMNS.index("ID сотрудника")] == str(telegram_id)
                and row[COLUMNS.index("Статус")] == STATUS_ACTIVE
            ):
                return (i + 1, row)
