...
from app.config.settings import (
    GOOGLE_SHEETS_ID, GOOGLE_CREDENTIALS_JSON_PATH, SHEET_NAME,
    COLUMNS, STATUS_ACTIVE, STATUS_FINISHED, DATETIME_FORMAT, DATE_FORMAT, logger,
)
...
class GoogleSheetsService:
    ...
    def _connect(self) -> None:
        ...
        credentials = Credentials.from_service_account_file(GOOGLE_CREDENTIALS_JSON_PATH, scopes=self.SCOPES)
    ...
    def _get_or_create_sheet(self) -> gspread.Worksheet:
        try:
            ...
            sheet = self._spreadsheet.add_worksheet(title=SHEET_NAME, rows=1000, cols=len(COLUMNS))
    ...
    def add_time_log(self, user_name: str, telegram_id: int, process: str, started_by: str) -> dict:
        with self._lock:
            ...
            row = {
                "ID": row_id, "Сотрудник": user_name, "ID сотрудника": str(telegram_id),
                "Процесс": process, "Начало": now.strftime(DATETIME_FORMAT), "Конец": "",
                "Длительность (мин)": "", "Статус": STATUS_ACTIVE,
                "Запустил": started_by, "Завершил": "", "Дата": now.strftime(DATE_FORMAT),
            }
    ...
    def finish_time_log(self, telegram_id: int, finished_by: str) -> Optional[dict]:
        with self._lock:
            ...
            for col_name, value in updates.items():
                self._sheet.update_cell(row_number, self._col_index(col_name), value)
    ...
    def get_active_process(self, telegram_id: int) -> Optional[dict]:
        ...
        return {"process": row_data[COLUMNS.index("Процесс")], "start_time": start_time_str, "duration_minutes": elapsed}
    ...
    def get_today_logs(self, telegram_id: int) -> list[dict]:
        ...
        for row in all_rows[1:]:
            ...
            if row[COLUMNS.index("ID сотрудника")] == str(telegram_id) and row[COLUMNS.index("Дата")] == today:
                results.append({"process": row[COLUMNS.index("Процесс")], "start_time": row[COLUMNS.index("Начало")], "end_time": row[COLUMNS.index("Конец")], "duration_minutes": row[COLUMNS.index("Длительность (мин)")], "status": row[COLUMNS.index("Статус")]})
        return results
    ...
    def get_all_active_processes(self) -> list[dict]:
        ...
        for row in all_rows[1:]:
            ...
            if row[COLUMNS.index("Статус")] == STATUS_ACTIVE:
                results.append({"telegram_id": row[COLUMNS.index("ID сотрудника")], "user_name": row[COLUMNS.index("Сотрудник")], "process": row[COLUMNS.index("Процесс")], "start_time": row[COLUMNS.index("Начало")]})
        return results
    ...
    def get_today_all_logs(self) -> list[dict]:
        ...
        for row in all_rows[1:]:
            ...
            if row[COLUMNS.index("Дата")] == today:
                results.append({"user_name": row[COLUMNS.index("Сотрудник")], "process": row[COLUMNS.index("Процесс")], "start_time": row[COLUMNS.index("Начало")], "end_time": row[COLUMNS.index("Конец")], "duration_minutes": row[COLUMNS.index("Длительность (мин)")], "status": row[COLUMNS.index("Статус")]})
        return results
