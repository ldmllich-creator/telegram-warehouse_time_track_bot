...
from app.bot.keyboards.inline_keyboards import (
    get_worker_select_keyboard,
    get_process_select_keyboard,
    CB_SELECT_WORKER,
    CB_DONE_SELECT,
    CB_SELECT_PROCESS,
    CB_CONFIRM_YES,
...
from app.utils.messages import (
    MSG_GROUP_SELECT_WORKERS,
    MSG_GROUP_SELECT_PROCESS,
    MSG_GROUP_STARTED,
    MSG_GROUP_FINISHED,
    MSG_GROUP_CONFLICT,
    MSG_ACCESS_DENIED,
    MSG_GROUP_NOBODY_SELECTED,
    MSG_GROUP_NO_ACTIVE_TO_FINISH,
    MSG_ERROR,
...
# /group_start
# ---------------------------------------------------------------------------
async def group_start_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Начало группового запуска: показать выбор сотрудников."""
    ...
    # Инициализация состояния в context.user_data
    context.user_data["group_selected"] = set()
    context.user_data["group_mode"] = "start"

    await update.message.reply_text(
        MSG_GROUP_SELECT_WORKERS,
        reply_markup=get_worker_select_keyboard(),
    )
...
# /group_finish
# ---------------------------------------------------------------------------
async def group_finish_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Начало группового завершения: показать выбор сотрудников."""
    user = update.effective_user
    ...
    await update.message.reply_text(
        MSG_GROUP_SELECT_WORKERS,
        reply_markup=get_worker_select_keyboard(finish_mode=True),
    )


# ---------------------------------------------------------------------------
# Callback: toggle выбора сотрудника (start mode)
# ---------------------------------------------------------------------------
async def toggle_worker_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Переключение выбора сотрудника в режиме group_start."""
    ...
    await query.edit_message_reply_markup(
        reply_markup=get_worker_select_keyboard(selected_ids=selected)
    )


# ---------------------------------------------------------------------------
# Callback: «Готово» после выбора сотрудников (start mode)
# ---------------------------------------------------------------------------
async def done_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """После выбора сотрудников — показать выбор процесса."""
    query = update.callback_query
    await query.answer()
    ...
    await query.edit_message_text(
        MSG_GROUP_SELECT_PROCESS,
        reply_markup=get_process_select_keyboard(),
    )


# ---------------------------------------------------------------------------
# Callback: выбор процесса
# ---------------------------------------------------------------------------
async def select_process_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Выбран процесс — выполнить групповой старт."""
    query = update.callback_query
    await query.answer()
    ...
    logger.info(
        "Групповой старт: supervisor=%s process=%s workers=%s",
        supervisor.full_name, process_key, selected,
    )
    ...
    for worker_id in selected:
        ...
        try:
            _sheets_service.add_time_log(
                user_name=worker["name"],
                telegram_id=worker["id"],
                process=process_display_name(process_key),
                started_by=f"supervisor: {supervisor.full_name}",
            )
    ...
    if results:
        text_parts.append(
            MSG_GROUP_STARTED.format(
                results="\n".join(results)
            )
        )
    if conflicts:
        text_parts.append(
            MSG_GROUP_CONFLICT.format(
...
# ---------------------------------------------------------------------------
# Callback: toggle выбора сотрудника (finish mode)
# ---------------------------------------------------------------------------
async def toggle_finish_worker_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Переключение выбора сотрудника в режиме group_finish."""
    query = update.callback_query
    await query.answer()
    ...
    await query.edit_message_reply_markup(
        reply_markup=get_worker_select_keyboard(
            selected_ids=selected, finish_mode=True
        )
    )


# ---------------------------------------------------------------------------
# Callback: «Готово» — выполнить групповое завершение
# ---------------------------------------------------------------------------
async def done_finish_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Выполнить групповое завершение процессов."""
    query = update.callback_query
    await query.answer()
    ...
    supervisor = update.effective_user
    logger.info(
        "Групповое завершение: supervisor=%s workers=%s",
        supervisor.full_name, selected,
    )
    ...
    for worker_id in selected:
        ...
        try:
            result = _sheets_service.finish_time_log(
                telegram_id=worker["id"],
                finished_by=f"supervisor: {supervisor.full_name}",
            )
            ...
            if result is None:
                ...
                results.append(
                    f"✅ {worker['name']} — "
                    f"{result['process']} "
                    f"({result['duration_minutes']} мин)"
                )
        except Exception as e:
            logger.exception(
                "Ошибка группового завершения для %s: %s", worker["name"], e
            )
            results.append(f"❌ {worker['name']} — ошибка")
    ...
    if results:
        text_parts.append(
            MSG_GROUP_FINISHED.format(results="\n".join(results))
        )
    if no_active:
        text_parts.append(
            MSG_GROUP_NO_ACTIVE_TO_FINISH
...
# Callback: отмена
# ---------------------------------------------------------------------------
async def confirm_no_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Отмена группового действия."""
    query = update.callback_query
    await query.answer()
...
def get_handlers() -> list:
    ...
    return [
        ...
        # Start mode callbacks
        CallbackQueryHandler(
            toggle_worker_callback,
            pattern=f"^{CB_SELECT_WORKER}\\d+$",
        ),
        CallbackQueryHandler(done_select_callback, pattern=f"^{CB_DONE_SELECT}$"),
        CallbackQueryHandler(
            select_process_callback,
            pattern=f"^{CB_SELECT_PROCESS}",
        ),
        # Finish mode callbacks
        CallbackQueryHandler(
            toggle_finish_worker_callback,
            pattern=f"^{CB_FINISH_WORKER}\\d+$",
        ),
